---
title: "How This Website Works (Part 1: Getting Connected)" 
date: 2025-11-13
...
### DNS Resolution: Getting My Phone Number
...
### The TCP/TLS Handshake: Establishing Encryption
...
CloudFront checks its cache, potentially queries S3 if it doesn't have it, and sends the HTML back to Bob. The browser parses that HTML, realizes it needs CSS, JavaScript, and images, and makes more requests. Rinse and repeat. The entire process—from typing the URL to seeing a fully rendered page—typically takes 80-150ms for an uncached request. Cached requests drop to 20-50ms. 

### That's Just the Beginning

Ok, so now Bob has my homepage. But I glossed over a lot of important stuff in that last paragraph because I got a little uninspired and wanted to pick this up later. CloudFront "checks its cache" -- what does that actually mean? What if it's not cached? How does content get from S3 to CloudFront in the first place? And how did the files get into S3? 

I'll eventually write a part 2 to talk about the build process, caching strategies, and how to build your own site that looks like this one.
