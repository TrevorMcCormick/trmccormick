...
def handler(event, context):
    ...
    try:
        ...
        # 5. Check for score drops and send alerts
        sns_topic_arn = os.environ.get('SNS_TOPIC_ARN', '')
        alert_threshold = float(os.environ.get('ALERT_THRESHOLD', 5))

        if sns_topic_arn:
            print("Checking for score drops...")
            previous_metrics = get_previous_metrics(table_name)
            if previous_metrics:
                check_and_alert_score_drops(
...
def get_previous_metrics(table_name):
    """
    Fetch the most recent previous metric from DynamoDB

    Args:
        table_name: DynamoDB table name

    Returns:
        dict: Previous metrics item or None if not found
    """
    dynamodb = boto3.resource('dynamodb')
    table = dynamodb.Table(table_name)
    ...
    try:
        response = table.query(
            ...
            Limit=2  # Get latest 2 (current will be stored after this check)
        )

        items = response.get('Items', [])
        if len(items) >= 1:
            # Return the most recent one (since we haven't stored the new one yet)
            return items[0]

        return None

    except Exception as e:
        print(f"Error fetching previous metrics: {str(e)}")
        return None


def check_and_alert_score_drops(previous_metrics, current_metrics, threshold, sns_topic_arn):
    """
    Compare current metrics with previous metrics and send SNS alert if scores drop

    Args:
        previous_metrics: Previous metrics from DynamoDB
        current_metrics: Current metrics to be stored
        threshold: Score drop threshold (in points) to trigger alert
        sns_topic_arn: SNS topic ARN to publish alerts
    """
    # Categories to check
    categories = ['performance_score', 'accessibility_score', 'best_practices_score', 'seo_score']
    strategies = ['mobile', 'desktop']

    alerts = []

    for strategy in strategies:
        prev_strategy = previous_metrics.get(strategy, {})
        curr_strategy = current_metrics.get(strategy, {})

        for category in categories:
            prev_score = float(prev_strategy.get(category, 0))
            curr_score = float(curr_strategy.get(category, 0))

            # Calculate drop (positive number means score decreased)
    ...
    # If we have alerts, send SNS notification
    if alerts:
        ...
        print("No significant score drops detected")


def send_alert_notification(alerts, current_metrics, previous_metrics, sns_topic_arn):
    """
    Send SNS notification about score drops

    Args:
        alerts: List of alert dictionaries
        current_metrics: Current metrics data
        previous_metrics: Previous metrics data
        sns_topic_arn: SNS topic ARN
    """
    sns = boto3.client('sns')

    # Build alert message
    subject = f"⚠️ PageSpeed Score Drop Alert - {len(alerts)} Issue(s) Detected"

    message_lines = [
        "PageSpeed Insights has detected significant score drops:\n",
        f"URL: {current_metrics.get('url', 'N/A')}",
        f"Current Commit: {current_metrics.get('commit', {}).get('short_sha', 'N/A')}",
        f"Previous Commit: {previous_metrics.get('commit', {}).get('short_sha', 'N/A')}\n",
        "Score Drops:\n"
    ]

    for alert in alerts:
        message_lines.append(
            f"  • {alert['strategy']} - {alert['category']}: "
            f"{alert['previous']:.1f} → {alert['current']:.1f} "
            f"(↓ {alert['drop']:.1f} points)"
        )
    ...
    for strategy in ['mobile', 'desktop']:
        strategy_data = current_metrics.get(strategy, {})
        message_lines.append(f"\n{strategy.title()}:")
        message_lines.append(f"  Performance:    {strategy_data.get('performance_score', 0):.1f}")
        message_lines.append(f"  Accessibility:  {strategy_data.get('accessibility_score', 0):.1f}")
        message_lines.append(f"  Best Practices: {strategy_data.get('best_practices_score', 0):.1f}")
        message_lines.append(f"  SEO:            {strategy_data.get('seo_score', 0):.1f}")

    commit_url = current_metrics.get('commit', {}).get('url', '')
    if commit_url:
        message_lines.append(f"\nCommit: {commit_url}")
    ...
    try:
        ...
        print(f"Detected {len(alerts)} score drop(s) exceeding threshold")
